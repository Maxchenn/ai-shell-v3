...
use tauri::{
    ...
    AppHandle, Manager, PhysicalPosition, PhysicalSize, Window,
};
...
impl Default for SavedSettings {
    fn default() -> Self {
        Self {
            ...
            sites: vec![
                Site { id: "deepseek".into(), name: "DeepSeek".into(), url: "https://chat.deepseek.com/".into(), builtin: true },
                Site { id: "chatgpt".into(), name: "ChatGPT".into(), url: "https://chatgpt.com/".into(), builtin: true },
                Site { id: "claude".into(), name: "Claude".into(), url: "https://claude.ai/".into(), builtin: true },
                Site { id: "gemini".into(), name: "Gemini".into(), url: "https://gemini.google.com/".into(), builtin: true },
            ],
            pinned: false,
...
fn state(app: &AppHandle) -> tauri::Result<Arc<AppState>> {
    app.try_state::<Arc<AppState>>()
        .ok_or_else(|| tauri::Error::Anyhow(anyhow::anyhow!("应用状态尚未初始化")))
}
...
fn load_settings(path: &PathBuf) -> SavedSettings {
    fs::read_to_string(path)
        ...
        .map(|mut s| {
            if s.sites.is_empty() { s = SavedSettings::default(); }
            if !s.sites.iter().any(|x| x.id == s.selected_site_id) {
                s.selected_site_id = s.sites.first().map(|x| x.id.clone()).unwrap_or_else(|| "deepseek".into());
            }
...
fn save_settings(app: &AppHandle) -> tauri::Result<()> {
    ...
    if let Some(parent) = st.save_path.parent() { fs::create_dir_all(parent)?; }
...
fn selected_site(app: &AppHandle) -> tauri::Result<Site> {
    ...
    settings.sites.iter().find(|x| x.id == settings.selected_site_id)
        .cloned()
        .ok_or_else(|| tauri::Error::Anyhow(anyhow::anyhow!("找不到当前网站")))
}
...
fn save_window_geometry(app: &AppHandle) -> tauri::Result<()> {
    let window = app.get_webview_window(WINDOW_LABEL)
        .ok_or_else(|| tauri::Error::Anyhow(anyhow::anyhow!("找不到主窗口")))?;
    if window.is_fullscreen().unwrap_or(false) { return Ok(()); }
...
fn layout_webviews(window: &Window, toolbar_height: u32) -> tauri::Result<()> {
    ...
    if let Some(wv) = window.get_webview(TOOLBAR_LABEL) {
        ...
        wv.set_size(PhysicalSize::new(size.width, toolbar_height.min(size.height)))?;
    }
    if let Some(wv) = window.get_webview(REMOTE_LABEL) {
        ...
        wv.set_size(PhysicalSize::new(size.width, size.height.saturating_sub(y)))?;
    }
    Ok(())
}
...
fn toggle_fullscreen(app: &AppHandle) -> tauri::Result<()> {
    ...
    if *fullscreen || window.is_fullscreen()? {
        ...
        *fullscreen = false;
        ...
        layout_webviews(window.as_ref(), height)?;
    } else {
...
fn navigate_to_selected(app: &AppHandle) -> tauri::Result<()> {
    ...
    let remote = app.get_webview(REMOTE_LABEL)
...
#[tauri::command]
fn add_site(app: AppHandle, name: String, url: String) -> tauri::Result<SavedSettings> {
    let parsed = Url::parse(&url)?;
    if !matches!(parsed.scheme(), "http" | "https") {
        return Err(tauri::Error::Anyhow(anyhow::anyhow!("只支持 HTTP/HTTPS 网站")));
    }
    let id = format!("custom-{}", uuid_like(&url));
    {
        ...
        if settings.sites.iter().any(|s| s.url == url) {
            return Err(tauri::Error::Anyhow(anyhow::anyhow!("这个网址已经存在")));
        }
        settings.sites.push(Site { id: id.clone(), name: name.trim().to_string(), url: url.clone(), builtin: false });
        settings.selected_site_id = id;
    }
    navigate_to_selected(&app)?;
    save_settings(&app)?;
...
fn remove_site(app: AppHandle, id: String) -> tauri::Result<SavedSettings> {
    let selected;
    {
        let st = state(&app)?;
        let mut settings = st.settings.lock().unwrap();
        if settings.sites.iter().find(|s| s.id == id).map(|s| s.builtin).unwrap_or(true) {
            return Err(tauri::Error::Anyhow(anyhow::anyhow!("内置网站不能删除")));
        }
        settings.sites.retain(|s| s.id != id);
        if settings.selected_site_id == id {
            settings.selected_site_id = settings.sites.first().map(|s| s.id.clone()).unwrap_or_else(|| "deepseek".into());
        }
...
fn set_toolbar_height(app: AppHandle, height: u32) -> tauri::Result<()> {
    let window = app.get_webview_window(WINDOW_LABEL)
        .ok_or_else(|| tauri::Error::Anyhow(anyhow::anyhow!("找不到主窗口")))?;
    state(&app)?.toolbar_height.lock().map(|mut h| *h = height).map_err(|_| tauri::Error::Anyhow(anyhow::anyhow!("工具栏状态锁定失败")))?;
    layout_webviews(window.as_ref(), height)
}
...
fn build_main_window(app: &tauri::AppHandle, settings: &SavedSettings) -> tauri::Result<Window> {
    ...
    let toolbar_builder = WebviewBuilder::new(TOOLBAR_LABEL, WebviewUrl::App("index.html".into()))
        .focused(true)
        .initialization_script("document.documentElement.dataset.aiShellToolbar='1';");
    window.add_child(toolbar_builder, PhysicalPosition::new(0, 0), PhysicalSize::new(inner.width, TOOLBAR_HEIGHT))?;

    let site = settings.sites.iter().find(|s| s.id == settings.selected_site_id).cloned()
        .unwrap_or_else(|| SavedSettings::default().sites[0].clone());
    let remote_url = Url::parse(&site.url)?;
    let remote_builder = WebviewBuilder::new(REMOTE_LABEL, WebviewUrl::External(remote_url))
        .initialization_script(REMOTE_INIT)
        .on_navigation({
            let app = app.clone();
            move |url| {
                if url.scheme() == "aishell" && url.host_str() == Some("fullscreen") {
    ...
    window.add_child(remote_builder, PhysicalPosition::new(0, TOOLBAR_HEIGHT), PhysicalSize::new(inner.width, inner.height.saturating_sub(TOOLBAR_HEIGHT)))?;
...
fn init_state(app: &tauri::AppHandle) -> tauri::Result<()> {
    ...
    let initial_size = PhysicalSize::new(settings.width.max(MIN_WIDTH), settings.height.max(MIN_HEIGHT));
    app.manage(Arc::new(AppState {
        settings: Mutex::new(settings.clone()),
        save_path: path,
        fullscreen: Mutex::new(false),
        saved_windowed_size: Mutex::new(initial_size),
        saved_windowed_position: Mutex::new(PhysicalPosition::new(settings.x, settings.y)),
...
fn make_tray(app: &tauri::AppHandle) -> tauri::Result<()> {
    ...
    TrayIconBuilder::with_id("main-tray")
        ...
        .on_menu_event(|app, event| match event.id().as_ref() {
            "show" => { let _ = toggle_visibility_inner(app); }
            "quit" => { let _ = save_window_geometry(app); app.exit(0); }
            _ => {}
        })
        .build(app)?;
    Ok(())
...
pub fn run() {
    ...
    builder = builder.setup(|app| {
        ...
        let window = build_main_window(app, &settings)?;
        make_tray(app)?;
        ...
        window.on_window_event(move |event| {
            match event {
                ...
                WindowEvent::Resized(_) => {
                    if let Ok(st) = state(&app_handle) {
                        ...
                        if let Some(w) = app_handle.get_webview_window(WINDOW_LABEL) {
                            let _ = layout_webviews(w.as_ref(), h);
                        }
                    }
                }
                WindowEvent::CloseRequested { api, .. } => {
                    ...
                    let _ = app_handle.get_webview_window(WINDOW_LABEL).map(|w| w.hide());
                }
